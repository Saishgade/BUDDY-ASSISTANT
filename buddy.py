...
try:
    ...
    print("ERROR: PyQt6 is required to run Buddy.")
...
# ==============================================================================
# OPENROUTER CLIENT (or_client.py logic)
# ==============================================================================

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("openrouter_client")

def _load_or_api_key() -> str:
    try:
        ...
        raise RuntimeError(f"api_keys.json not found at: {API_CONFIG_PATH}")
...
DEFAULT_WELCOME_SPEECH = (
    "System initialization complete. Please warmly welcome me "
    "by saying: 'Jay Shree Ram Saish. How may I help you today?'" )
...
class OpenRouterClient:
    ...
    def __init__(self) -> None:
        self.api_key  = _load_or_api_key()
        self._headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type":  "application/json",
            "HTTP-Referer":  "https://github.com/ai-assistant",
            "X-Title":       "BUDDY",
        }
...
RAINBOW_COLORS = [
    "#ff0044", "#ff7700", "#ffd300", "#00cc44",
    "#00aaff", "#8800ff", "#ff00cc"
]
...
def rainbow_text(text: str) -> str:
    return "".join(
        f'<span style="color: {RAINBOW_COLORS[i % len(RAINBOW_COLORS)]}">{ch}</span>'
        if ch != " " else " "
...
class HudCanvas(QWidget):
    ...
    def paintEvent(self, _):
        ...
        p.drawText(QRectF(cx - 80, cy - 14, 160, 28),
                   Qt.AlignmentFlag.AlignCenter, "B.U.D.D.Y")
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure systems before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
        ...
        layout.addSpacing(8)
        layout.addWidget(_lbl("WELCOME MESSAGE", 8, color=C.TEXT_DIM,
                               align=Qt.AlignmentFlag.AlignLeft))
        self._welcome_input = QLineEdit()
        self._welcome_input.setPlaceholderText(
            "Hello! I'm BUDDY, your assistant. How can I help today?")
        self._welcome_input.setFont(QFont("Courier New", 10))
        self._welcome_input.setFixedHeight(32)
        self._welcome_input.setStyleSheet(f"""
            QLineEdit {{
                background: #000d12; color: {C.TEXT};
                border: 1px solid {C.BORDER}; border-radius: 3px; padding: 4px 8px;
            }}
            QLineEdit:focus {{ border: 1px solid {C.ACC2}; }}
        """)
        layout.addWidget(self._welcome_input)

        layout.addSpacing(8)
        layout.addWidget(_lbl("VOICE PACK", 8, color=C.TEXT_DIM,
                               align=Qt.AlignmentFlag.AlignLeft))
        self._voice_input = QLineEdit()
        self._voice_input.setPlaceholderText("Charon")
        self._voice_input.setFont(QFont("Courier New", 10))
        self._voice_input.setFixedHeight(32)
        self._voice_input.setStyleSheet(f"""
            ...
            QLineEdit:focus {{ border: 1px solid {C.ACC2}; }}
        """)
        layout.addWidget(self._voice_input)
    ...
    def _submit(self):
        ...
        welcome = self._welcome_input.text().strip()
        voice = self._voice_input.text().strip()
        if not key:
        ...
        if not or_key:
            ...
            return
        self.done.emit(key, or_key, self._sel_os, welcome, voice)
...
class MainWindow(QMainWindow):
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("V1", C.PRI_DIM))
        lay.addStretch()

        mid = QVBoxLayout(); mid.setSpacing(1)
        title = QLabel(rainbow_text("B.U.D.D.Y"))
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        title.setFont(QFont("Courier New", 17, QFont.Weight.Bold))
        title.setStyleSheet("background: transparent;")
        mid.addWidget(title)
        sub = QLabel(rainbow_text("Intelligent System"))
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("© CORE SYSTEMS", C.PRI_DIM))
        return w
    ...
    def _on_setup_done(self, key: str, or_key: str, os_name: str, welcome_message: str = "", voice_name: str = ""):
...
# ==============================================================================
# MAIN APPLICATION (main.py logic)
# ==============================================================================
...
def _get_welcome_message() -> str:
    try:
        ...
        return data.get("welcome_message", DEFAULT_WELCOME_SPEECH) or DEFAULT_WELCOME_SPEECH
    except Exception:
        return DEFAULT_WELCOME_SPEECH


def _get_voice_name() -> str:
    try:
        data = json.loads(API_CONFIG_PATH.read_text(encoding="utf-8"))
        return data.get("voice_name", DEFAULT_VOICE_NAME) or DEFAULT_VOICE_NAME
    except Exception:
        return DEFAULT_VOICE_NAME


def _set_voice_name(name: str) -> None:
    try:
        data = json.loads(API_CONFIG_PATH.read_text(encoding="utf-8"))
    except Exception:
        data = {}
    ...
    try:
        ...
        print(f"[BUDDY] ⚠️ Failed to save voice_name: {exc}")


_last_memory_input = ""


def _update_memory_async(user_text: str, buddy_text: str) -> None:
    global _last_memory_input

    user_text   = (user_text   or "").strip()
    buddy_text  = (buddy_text or "").strip()
    ...
    try:
        ...
        if not should_extract_memory(user_text, buddy_text, api_key):
            return
        data = extract_memory(user_text, buddy_text, api_key)
        if data:
            update_memory(data)
...
class BuddyLive:
    ...
    def speak_error(self, tool_name: str, error: str):
        ...
        self.speak(f"The tool {tool_name} encountered an error. {short}")
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                self.ui.set_state("THINKING")
                config = self._build_config(voice_name=voice_name)
                ...
                async with client.aio.live.connect(model=LIVE_MODEL, config=config) as session:
                    ...
                    # --- "Jay Shree Ram Saish" "How may i help you today" ---
                    self.speak(_get_welcome_message())
                    # -------------------------------------------
                ...
                message = str(e)
                if "Requested voice api_name" in message and voice_name != DEFAULT_VOICE_NAME:
                    print(f"[BUDDY] ⚠️ Voice '{voice_name}' invalid, falling back to '{DEFAULT_VOICE_NAME}'.")
                    voice_name = DEFAULT_VOICE_NAME
                    _set_voice_name(DEFAULT_VOICE_NAME)
